--TEST--
Swoole\Async\Client: getSocket debug
--SKIPIF--
...
$cli->on("connect", function (Swoole\Async\Client $cli) use ($timer) {
    ...
    @$cli->getSocket();
    $err = error_get_last();
    Assert::same($err["message"], "Swoole\Async\Client_async::getSocket(): unable to obtain socket family Error: Bad file descriptor[9].");
    */
    $cli->close();
