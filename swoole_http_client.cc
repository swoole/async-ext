...
static void http_client_onRequestTimeout(swTimer *timer, TimerNode *tnode);
...
static int http_client_execute(zval *zobject, char *uri, size_t uri_len, zval *callback)
{
    ...
    if (zset && ZVAL_IS_ARRAY(zset))
    {
        ...
        if (http->cli->http_proxy)
        {
            ...
            if (!http->cli->http_proxy->password.empty())
            {
                char _buf1[128];
                char _buf2[256];
...
static int http_client_send_http_request(zval *zobject)
{
    ...
    http_client_buffer->append(http->uri, http->uri_len);
    http_client_buffer->append(ZEND_STRL(" HTTP/1.1\r\n"));
    ...
    if (zsend_header && Z_TYPE_P(zsend_header) == IS_ARRAY)
    {
        ...
        {
            http_client_append_headers(http_client_buffer, ZEND_STRL("Host"), Z_STRVAL_P(value), Z_STRLEN_P(value));
        }
        else
        {
            http_client_append_headers(http_client_buffer, ZEND_STRL("Host"), http->host, http->host_len);
        }
        ...
        SW_HASHTABLE_FOREACH_START2(Z_ARRVAL_P(zsend_header), key, keylen, keytype, value)
            ...
            if (ZSTR_LEN(str) > 0)
            {
                http_client_append_headers(http_client_buffer, key, keylen, ZSTR_VAL(str), ZSTR_LEN(str));
            }
            zend_string_release(str);
    ...
    if (!(header_flag & HTTP_HEADER_CONNECTION))
    {
        if (http->keep_alive)
        {
            http_client_append_headers(http_client_buffer, ZEND_STRL("Connection"), ZEND_STRL("keep-alive"));
        }
        else
        {
            http_client_append_headers(http_client_buffer, ZEND_STRL("Connection"), ZEND_STRL("closed"));
        }
    }
#ifdef SW_HAVE_ZLIB
    if (!(header_flag & HTTP_HEADER_ACCEPT_ENCODING))
    {
        http_client_append_headers(http_client_buffer, ZEND_STRL("Accept-Encoding"), ZEND_STRL("gzip"));
    }
#endif
    ...
    {
        http_client_buffer->append(ZEND_STRL("Cookie: "));
        ...
        SW_HASHTABLE_FOREACH_START2(Z_ARRVAL_P(zcookies), key, keylen, keytype, value)
            ...
            http_client_buffer->append(key, keylen);
            http_client_buffer->append("=", 1);

            int encoded_value_len;
            encoded_value = php_swoole_url_encode(ZSTR_VAL(str), ZSTR_LEN(str), &encoded_value_len);
            if (encoded_value)
            {
                http_client_buffer->append(encoded_value, encoded_value_len);
                efree(encoded_value);
            }
            if (i < n_cookie)
            {
                http_client_buffer->append("; ", 2);
            }
            zend_string_release(str);
        SW_HASHTABLE_FOREACH_END();
        http_client_buffer->append(ZEND_STRL("\r\n"));
    ...
    if (hcc->request_upload_files)
    {
        ...
        http_client_buffer->append(header_buf, n);
        ...
        if (Z_TYPE_P(zpost_data) == IS_ARRAY && php_swoole_array_length(zpost_data) > 0)
        {
            SW_HASHTABLE_FOREACH_START2(Z_ARRVAL_P(zpost_data), key, keylen, keytype, value)
                ...
                http_client_buffer->append(header_buf, n);
                http_client_buffer->append(ZSTR_VAL(str), ZSTR_LEN(str));
                http_client_buffer->append(ZEND_STRL("\r\n"));
                zend_string_release(str);
            SW_HASHTABLE_FOREACH_END();
    ...
    else if (zpost_data && !ZVAL_IS_NULL(zpost_data))
    {
        ...
        {
            size_t len;
            http_client_append_headers(http_client_buffer, ZEND_STRL("Content-Type"), ZEND_STRL("application/x-www-form-urlencoded"));
            if (php_swoole_array_length(zpost_data) > 0) //if it's an empty array, http build will fail
            {
                ...
                http_client_buffer->append(formstr, len);
    ...
    if (http->timeout > 0)
    {
        http->timer = swoole_timer_add((long) (http->timeout * 1000), 0, http_client_onRequestTimeout, http->cli);
    }
...
static PHP_METHOD(swoole_http_client, __construct)
{
    ...
    zend_update_property_stringl(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("host"), host, host_len);
    zend_update_property_long(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("port"), port);
    zend_update_property_bool(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("ssl"), ssl);
    ...
    swoole_set_property(ZEND_THIS, 0, hcc);
    ...
    zend_update_property_long(swoole_client_ce, ZEND_THIS, ZEND_STRL("type"), flags);
...
static PHP_METHOD(swoole_http_client, __destruct)
{
    ...
    http_client *http = (http_client *) swoole_get_object(ZEND_THIS);
    if (http && http->cli)
    {
        zval *zobject = ZEND_THIS;
        sw_zend_call_method_with_0_params(zobject, swoole_http_client_ce, NULL, "close", NULL);
    }
    http_client_property *hcc = (http_client_property *) swoole_get_property(ZEND_THIS, 0);
    if (hcc)
    {
        if (hcc->onResponse)
        {
...
static PHP_METHOD(swoole_http_client, set)
{
    ...
    zval *zsetting = sw_zend_read_and_convert_property_array(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("setting"), 1);
...
static PHP_METHOD(swoole_http_client, setHeaders)
{
    ...
    zval *zheaders = sw_zend_read_and_convert_property_array(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("requestHeaders"), 1);
...
static PHP_METHOD(swoole_http_client, setCookies)
{
    ...
    zval *zcookies = sw_zend_read_and_convert_property_array(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("cookies"), 1);
...
static PHP_METHOD(swoole_http_client, setData)
{
    ...
    {
        zval *zbody = sw_zend_read_and_convert_property_array(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("requestBody"), 1);
        php_array_merge(Z_ARRVAL_P(zbody), Z_ARRVAL_P(data));
    }
    else
    {
        zend_update_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("requestBody"), data);
    }
    RETURN_TRUE;
}
...
static PHP_METHOD(swoole_http_client, addFile)
{
    ...
    {
        array_init(&files);
        zend_update_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("uploadFiles"), &files);
        zval_ptr_dtor(&files);

        hcc->request_upload_files = sw_zend_read_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("uploadFiles"), 0);
...
static PHP_METHOD(swoole_http_client, setMethod)
{
    ...
    zend_update_property_string(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("requestMethod"),
...
static PHP_METHOD(swoole_http_client, on)
{
    ...
    {
        zend_update_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("onError"), zcallback);
        hcc->onError = sw_zend_read_property(swoole_http_client_ce,  ZEND_THIS, ZEND_STRL("onError"), 0);
        sw_copy_to_stack(hcc->onError, hcc->_onError);
    }
    else if (strncasecmp("connect", cb_name, cb_name_len) == 0)
    {
        zend_update_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("onConnect"), zcallback);
        hcc->onConnect = sw_zend_read_property(swoole_http_client_ce,  ZEND_THIS, ZEND_STRL("onConnect"), 0);
        sw_copy_to_stack(hcc->onConnect, hcc->_onConnect);
    }
    else if (strncasecmp("close", cb_name, cb_name_len) == 0)
    {
        zend_update_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("onClose"), zcallback);
        hcc->onClose = sw_zend_read_property(swoole_http_client_ce,  ZEND_THIS, ZEND_STRL("onClose"), 0);
        sw_copy_to_stack(hcc->onClose, hcc->_onClose);
    }
    else if (strncasecmp("message", cb_name, cb_name_len) == 0)
    {
        zend_update_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("onMessage"), zcallback);
        hcc->onMessage = sw_zend_read_property(swoole_http_client_ce,  ZEND_THIS, ZEND_STRL("onMessage"), 0);
...
static int php_swoole_zlib_uncompress(z_stream *stream, swString *buffer, char *body, int length)
{
    int status = 0;
...
static PHP_METHOD(swoole_http_client, execute)
{
    ...
    ret = http_client_execute(ZEND_THIS, uri, uri_len, finish_cb);
    SW_CHECK_RETURN(ret);
}
...
static PHP_METHOD(swoole_http_client, download)
{
    ...
    zend_update_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("downloadFile"), download_file);
    hcc->download_file = sw_zend_read_property(swoole_http_client_ce, ZEND_THIS, ZEND_STRL("downloadFile"), 1);
...
static PHP_METHOD(swoole_http_client, post)
{
    ...
    ret = http_client_execute(ZEND_THIS, uri, uri_len, callback);
    SW_CHECK_RETURN(ret);
}
